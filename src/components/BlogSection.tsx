...
export const BlogSection = component$(() => {
  const blogs = useSignal<Blog[]>([
    {
      ...
      short:"AI-powered home automation is more than just a convenience in today's technologically advanced world; it's a calculated investment that raises the real estate market's value. Smart automation technologies have become a potent differentiator as companies in the home construction and real estate sectors search for creative methods to draw clients and boost asset value.",
    ...
    },
    {
      ...
      short:"A paradigm shift in facility management has been brought about by the use of AI into smart home automation. The use of AI in residential and commercial property management has grown in popularity as companies aim for operational excellence, redefining roles, efficiencies, and value generation.",
    ...
    },
    {
      ...
      short:"AI gathers and examines enormous volumes of data from networked devices. These insights can be used by facility managers to estimate energy requirements, plan upgrades, and improve operations. Facility management is transformed from a cost center into a strategic function by this data-centric strategy.",
  ...
  const currentBlog = useSignal<Blog | null>(blogs.value[0]); // Initialize with the first blog

  useTask$(async ({ track }) => {
    track(currentIndex);
    if (blogs.value.length > 0) {
      currentBlog.value = blogs.value[currentIndex.value];
    }
  });
      ...
      <div class="w-11/12 backdrop-blur-xl text-emerald-50 p-8 rounded-3xl z-10 grid m-4 grid-cols-1 md:grid-cols-5 space-x-4">
        ...
        <div class="flex col-span-2 flex-col gap-2">
          <p
            class="text-sm"
            >
            {currentBlog.value.short.slice(0, 200)}...
            </p>
          <Link
